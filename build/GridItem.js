...
var GridItem = function (_React$Component) {
  ...
  GridItem.prototype.getBodyScale = function getBodyScale() {
    var regExp = /scale\(([^)]+)\)/;
    var bodyTransform = document && document.body ? document.body.style.transform : '';
    var matches = regExp.exec(bodyTransform);
        ...
        case "onDragStart":
          {
            ...
            newPosition.left = (clientRect.left - parentRect.left + offsetParent.scrollLeft) / scale;
            newPosition.top = (clientRect.top - parentRect.top + offsetParent.scrollTop) / scale;
            _this2.setState({ dragging: newPosition });
            break;
          }
        case "onDrag":
          if (!_this2.state.dragging) throw new Error("onDrag called before onDragStart.");
          newPosition.left = _this2.state.dragging.left + deltaX / scale;
          newPosition.top = _this2.state.dragging.top + deltaY / scale;
          _this2.setState({ dragging: newPosition });
          break;
        case "onDragStop":
